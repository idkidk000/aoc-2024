...
func part2(program[]int,regB uint64,regC uint64, debug int){
	...
	for instruction:=len(program)-1;instruction>-1;instruction--{
		...
		for _,prevInput:=range inputs{
			for nextByte:=0;nextByte<8;nextByte++{
				nextInput:=(prevInput<<uint64(3))+uint64(nextByte);
...
func comboOperand(operand int,regA uint64,regB uint64,regC uint64)(uint64){
	switch operand{
		...
		case 5: return regB
		case 6: return regC
		case 7: fmt.Printf("invalid operand %d",operand) //no such thing as exceptions
	}
...
func parseArgs()(filename string,debug int){
	...
	for _,arg:=range os.Args[1:]{
		switch arg{
			...
			case "-e2": filename="example2.txt"
			case "-e3": filename="example3.txt"
			case "-e4": filename="example4.txt"
			case "-d": debug=1
			case "-d2": debug=2
			case "-d3": debug=3
			default: fmt.Printf("unknown arg: %s\n",arg)
		}
	}
	return
}
