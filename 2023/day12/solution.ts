...
const countArrangements = (record: SpringRecord, cache: Map<string, number> = new Map()): number => {
  const cacheKey = record.data.join('') + ' ' + record.checksums.join(',');
  ...
  if (record.checksums.length === 0) {
    ...
    cache.set(cacheKey, record.data.includes(State.Damaged) ? 0 : 1);
    return cache.get(cacheKey)!;
  }
  ...
  if ([State.Operational, State.Indeterminate].includes(record.data[0]))
    count += countArrangements({ data: record.data.slice(1), checksums: record.checksums }, cache);

  if ([State.Damaged, State.Indeterminate].includes(record.data[0])) {
    if (record.checksums[0] > record.data.length) {
      debug(2, 'data too short to satisfy checksum');
    } else if (record.data.slice(0, record.checksums[0]).includes(State.Operational)) {
      debug(2, 'operational in checksummed data');
    } else if (record.checksums[0] === record.data.length || record.data[record.checksums[0]] !== State.Damaged) {
      debug(2, 'valid');
...
const part2 = () => {
  ...
  const count = records
    .map((record) =>
      countArrangements({
        data: Array(5)
          .fill([...record.data, State.Indeterminate])
          .flat()
